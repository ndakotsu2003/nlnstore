					...
					<div class="col-md-12">
						<div class="row ">
							<div class="form-group col-md-5">
								...
								<select name="supplier_id" id="supp" class="custom-select browser-default select2">
									<option value=""></option>
								<?php 
							...
							<div class="col-md-3">
							<label class="control-label">SRV</label>
								<input type="text" name="srv" value="" class= "form-control text-right" placeholder="SRV" id ="srv">
								</div>
						</div>
						<hr>
						<div class="row mb-3">
								<div class="col-md-4">
									...
									while($row=$product->fetch_assoc()):
										$prod[$row['id']] = $row;
									?>
										<option value="<?php echo $row['id'] ?>" data-name="<?php echo $row['name'] ?>" data-description="<?php echo $row['description'] ?>"><?php echo $row['name'] . ' | ' . $row['bin_no'] ?></option>
									<?php endwhile; ?>
									</select>
								</div>
								<div class="col-md-2">
							...
							<table class="table table-bordered" id="list">
								...
								<tbody>
									...
									?>
										<tr class="item-row">
											<td>
												<p class ="srv text-right"><?php echo $row['ref_no'] ?></p>
											</td>
											<td>
...
<div id="tr_clone">
	...
	<tr class="item-row">
	    <td>
		    <p class =" srv text-right" name="srv2"></p>
		</td>
		<td>
...
<script>
	...
	$('#manage-receiving').submit(function(e){
		...
		if($("#list .item-row").length <= 0){
			alert_toast("Please insert an item first.",'danger');
			end_load();
			return false;
		}
		$.ajax({
