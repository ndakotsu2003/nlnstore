<?php include('db_connect.php');
	...
	$i = 1;
	while($i == 1){
		$chk = $conn->query("SELECT * FROM product_list where bin_no ='$sku'")->num_rows;
		if($chk > 0){
			$sku = mt_rand(1,99999999);
			$sku = sprintf("%'.08d\n", $sku);
		}else{
				...
				<div class="card">
					...
					<div class="card-body">
							<div class="form-group">
								...
								<select name="category_id" id="cat" class="custom-select browser-default">
								<option value=" "></option>
								<?php 
						...
						<div class="form-group">
							...
							<input type="text" id =" p_name" class="form-control" name="name" >
						</div>
						<div class="form-group">
						...
						<div class="form-group">
							<!-- <label class="control-label">Product Price</label> -->
							<input type="hidden" step="any" class="form-control text-right" value="1" name="price" >
						</div>
						...
						<table class="table table-bordered table-hover">
							...
							<tbody>
								...
								<tr>
									...
									<td class="">
										<p>BIN Number : <b><?php echo $row['bin_no'] ?></b></p>
...
<script>
	...
	$('#manage-product').submit(function(e){
		...
		if($("#cat").selectedIndex == 0 && $('#p_name').val().length <= 0 && $('#description').val().length <=0){
			alert_toast("Please fill in all the fields",'danger');
			end_load();
			return false;
		}
	...
	$('.edit_product').click(function(){
		...
		cat.find("[name='bin_no']").val($(this).attr('data-bin_no'))
